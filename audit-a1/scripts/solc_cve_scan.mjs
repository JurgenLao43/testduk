...
for (const f of entries) {
  try {
    ...
    const rawVer = j?.compiler?.version || j?.compilerVersion || '';
    const ver = parseVersion(rawVer);
    const flags = [];
    if (ver.major === 0 && ver.minor === 8 && ver.patch < 3) flags.push('solc<0.8.3: historical optimizer/sha3 issues');
    ...
    if (flags.length) risks.push({ file: f, compiler: rawVer || `${ver.major}.${ver.minor}.${ver.patch}`, flags });
  } catch {}
}
